...
class MiloOnboardingScreen extends StatefulWidget {
  ...
  @override
  State<MiloOnboardingScreen> createState() => _MiloOnboardingScreenState();
}
...
class _MiloOnboardingScreenState extends State<MiloOnboardingScreen> {
  ...
  Future<void> _create() async {
    ...
    try {
      await widget.onCreateFamily?.call(name);
  ...
  Future<void> _join() async {
    ...
    try {
      await widget.onJoinFamily?.call(code);
      if (mounted) Navigator.of(context).pop();
    } finally {
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _panel(
            ...
            subtitle: 'Gemeinsame Aufgaben, Termine und Zahlungen an einem Ort.',
            child: Column(children: [
              TextField(controller: familyController, textInputAction: TextInputAction.done, decoration: const InputDecoration(labelText: 'Name eurer Familie', hintText: 'z. B. Familie Aichberger', prefixIcon: Icon(CupertinoIcons.house))),
          ...
          _panel(
            ...
            subtitle: 'Du hast bereits eine Einladung von einem Familienmitglied?',
            child: Column(children: [
              TextField(controller: inviteController, textCapitalization: TextCapitalization.characters, decoration: const InputDecoration(labelText: 'Einladungscode', hintText: 'z. B. MILO-7K4P', prefixIcon: Icon(CupertinoIcons.number))),
              const SizedBox(height: 12),
  ...
  Widget _panel({required IconData icon, required String title, required String subtitle, required Widget child}) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [Container(width: 42, height: 42, decoration: BoxDecoration(color: const Color(0xFFE5EEE8), borderRadius: BorderRadius.circular(13)), child: Icon(icon, color: const Color(0xFF2E6A4F))), const SizedBox(width: 12), Expanded(child: Text(title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w750)))]),
      const SizedBox(height: 8),
